//     _____
//    ╱     ╲┌────╭─╭──────╭─────────╮────╭─╮
//   ╱  ╲ ╱  ╲ ┌──╮ │ ╭──╮ ╮ ┬─╮ ┬─╮ ┌ ┌──╮ │
//  ╱    Y    ╲└──╰ ╵ ╰──╯ │ │ │ │ │ │ └──╰ │
//  ╲____│____╱╰──╰─┴╭───╯ ╰─╰─╯ ╰─╰─┴────╰─┴╲╲
//            ╲╭─────┘─────└────────────────╮╱╱
//
// Copyright (C) esentri.magma - All Rights Reserved.
//
// Unauthorized copying of this file, via any medium
// is strictly prohibited. Proprietary and confidential.

package magma.exa.data.compute.pipe;

import magma.exa.data.compute.HPipe;
import magma.exa.value.scalar.I32;

/**
 * Pipe specialized for {@code int} values.
 */
public interface I32Pipe extends HPipe<Integer>, I32.Consumer {

    /**
     * Accepts {@code int} values.
     */
    @Override
    void onAccept(int value) throws Throwable;

    // ----------------------------------------------------------
    //  I32-PIPE.CHAINED
    // ----------------------------------------------------------

    /**
     * Chaining of a {@code int}-based input pipe with some output pipe.
     *
     * @param <P> type of output pipe.
     */
    abstract class Chained<P extends HPipe<?>>
            extends HPipe.Chained<Integer, P>
            implements I32Pipe {

        /**
         * Constructs a chain to the given output pipe.
         *
         * @param out output pipe to be chained.
         */
        protected Chained(final P out) {
            super(out);
        }
    }
}
